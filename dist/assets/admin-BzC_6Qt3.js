...
`)}</textarea>
  ...
  `).join(``),s(),e.certificates||=JSON.parse(JSON.stringify(window.defaultSiteData.certificates)),document.getElementById(`certs_tag`).value=e.certificates.tag,document.getElementById(`certs_title`).value=e.certificates.title,document.getElementById(`certs_lead`).value=e.certificates.lead,c(),document.getElementById(`values_tag`).value=e.values.tag,document.getElementById(`values_title`).value=e.values.title;let i=document.getElementById(`values_list_container`);i.innerHTML=e.values.list.map((e,t)=>`
  ...
  `).join(``)}function c(){let t=document.getElementById(`admin_certs_list`);if(!t)return;let n=e.certificates&&e.certificates.list||[];if(!n.length){t.innerHTML=`<p style="font-size:13px; color:var(--grey);">Henüz belge eklenmedi. "Yeni Belge Ekle" ile başlayın.</p>`;return}t.innerHTML=n.map((e,t)=>`
    <div class="project-list-item">
      <div class="proj-info">
        <img class="proj-thumb" src="${e.image}" alt="">
        <div class="proj-details">
